package infinity.com.sampleutils.data.modules.local.emulator.emulatorDetector;
...
//        }).start();
    ...
    private boolean detectOS() {
        ...
        if (!result) {
            result = checkBasic();
            log("EmulatorCheck basic " + result);
        }

        // EmulatorCheck Advanced
        if (!result) {
            result = checkAdvanced();
            log("EmulatorCheck Advanced " + result);
        }

        // EmulatorCheck Package Name
        if (!result) {
            result = checkPackageName();
            log("EmulatorCheck Package Name " + result);
        }
    ...
    private boolean checkDeviceId() {
        ...
        for (String known_deviceId : DEVICE_IDS) {
            if (known_deviceId.equalsIgnoreCase(deviceId)) {
                log("EmulatorCheck device id is detected");
                return true;
            }
    ...
    private boolean checkImsi() {
        ...
        for (String known_imsi : IMSI_IDS) {
            if (known_imsi.equalsIgnoreCase(imsi)) {
                log("EmulatorCheck imsi is detected");
                return true;
            }
        }
        return false;
    ...
    private boolean checkOperatorNameAndroid() {
        ...
        if (operatorName.equalsIgnoreCase("android")) {
            log("EmulatorCheck operator name android is detected");
            return true;
        }
        return false;
    }
            ...
            if (drivers_file.exists() && drivers_file.canRead()) {
                ...
                for (String known_qemu_driver : QEMU_DRIVERS) {
                    if (driver_data.contains(known_qemu_driver)) {
                        log("EmulatorCheck QEmuDrivers is detected");
                        return true;
                    }
                }
            }
    ...
    private boolean checkFiles(String[] targets, String type) {
        for (String pipe : targets) {
            ...
            if (qemu_file.exists()) {
                log("EmulatorCheck " + type + " is detected");
                return true;
            }
        }
        return false;
    ...
    private boolean checkQEmuProps() {
        ...
        if (found_props >= MIN_PROPERTIES_THRESHOLD) {
            log("EmulatorCheck QEmuProps is detected");
            return true;
        }
        return false;
    }
            ...
            if (!TextUtils.isEmpty(netData)) {
                ...
                for (String lan :
                    ...
                    if ((lan.contains("wlan0") || lan.contains("tunl0") || lan.contains("eth0"))
                            && lan.contains(IP)) {
                        ipDetected = true;
                        log("EmulatorCheck IP is detected");
                        break;
                    }
                }
